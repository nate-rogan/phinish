...
from phinish.scrape.types import Show, SongCatalogEntry, SongEntry
...
def _show(show_date: str, venue: str, sets: dict[str, list[str]]) -> Show:
    built_sets = {k: [_song(s, i) for i, s in enumerate(songs)] for k, songs in sets.items()}
    return Show(
        show_id=show_date,
        date=show_date,
        ...
        is_halloween=False,
        is_festival=False,
        total_songs=sum(len(v) for v in built_sets.values()),
    )
