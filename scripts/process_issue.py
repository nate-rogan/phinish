...
def format_comment(prediction: Prediction) -> str:
    ...
    Builds a header with the venue / date, one ``### Set N`` table per
    non-empty set (with confidence emoji + percentage + gap), and a
    footer summarizing the average confidence and the active ensemble
    weights. The footer's "Generated by Phinish" link uses GitHub's
    relative ``../tree/main`` form so it works on any fork of the repo.
    ...
    -------
    str
        Markdown body suitable for ``post_issue_comment`` or printing to
        stdout for local debugging. Sets that the model assigned no songs
