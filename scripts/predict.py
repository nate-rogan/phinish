...
LAST_SHOW_PENALTY = 0.0  # excluded entirely
LAST_3_PENALTY = 0.3
MARKOV_BLEND = 0.5
...
def _sequence(
    ...
    pool = {s: sc for s, sc in pool_scores}
    ...
    # Opener: highest combined ensemble + set-opener probability
    opener = max(pool, key=lambda s: 0.5 * pool[s] + 0.5 * set_openers.get(s, 0.0))
    sequence = [opener]
    pool.pop(opener)
    ...
    if pool and target_len >= 2:
        closer = max(pool, key=lambda s: 0.5 * pool[s] + 0.5 * set_closers.get(s, 0.0))
        sequence.append(closer)

    return sequence[:target_len]


def predict(date: str, venue: str, city: str | None = None) -> dict[str, Any]:
    ...
    X = np.array([featurize(state, show, song, cover_set) for song in candidates], dtype=np.float32)
    ...
    last_show_set = state.last_show_set
    last_3_set = {s for show_set in state.last_3_shows for s in show_set}

    scored: list[dict] = []
    for i, song in enumerate(candidates):
        if song in last_show_set:
            continue
        ensemble = wx * xgb_n[i] + wm * m_n[i] + wg * g_n[i] + wv * v_n[i]
        if song in last_3_set:
            ensemble *= LAST_3_PENALTY
        scored.append({
            "song": song,
            "ensemble": ensemble,
            "confidence": float(xgb_probs[i]),
            "gap": gaps.get(song, {}).get("gap", 0),
        })
    ...
    for set_key, target in SET_TARGETS.items():
        ...
        # Backfill from other buckets if a set is too small
        if len(pool) < target * 2:
            extras = [(s["song"], s["ensemble"]) for s in scored
                      if s["song"] not in used and s["song"] not in {p[0] for p in pool}]
    ...
    return {
        ...
        "model_version": weights.get("val_year", "unknown"),
        "weights": {"xgboost": wx, "markov": wm, "gap": wg, "venue": wv},
    }
